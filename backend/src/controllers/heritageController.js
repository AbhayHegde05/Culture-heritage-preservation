...
// @desc    Get all heritage sites (public: only verified)
...
exports.getHeritageSites = async (req, res, next) => {
  try {
    ...
    // If user is authenticated and requesting their own pending, allow it
    if (req.query.showPending === 'true' && req.user) {
      delete query.status;
      query.contributedBy = req.user.id;
    }
